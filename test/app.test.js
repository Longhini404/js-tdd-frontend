...
describe("#App", () => {
  ...
  describe("#isNameValid should validate if name has value", () => {
    ...
    test("given an valid name it should return false", () => {
      const appInstance = new App({});
      const result = appInstance.isNameValid("Guilherme Longhini");
  ...
  test("#update table should add new lines on top of to tBody elements HTML", () => {
    ...
    appInstance.updateTable("GuilhermeLonghini");
    appInstance.updateTable("Bruna");
  ...
  test("#cleanNameInput should cleanup name element value", () => {
    const appInstance = new App({});
    appInstance.name.value = "Guilherme Longhini";
  ...
  describe("#onButtonClick", () => {
    test("#onButtonClick should call updateTable and cleanNameInput if name is valid", () => {
      const appInstance = new App({});
      jest
        .spyOn(appInstance, appInstance.updateTable.name)
        .mockImplementation(() => {});
      jest
        .spyOn(appInstance, appInstance.cleanNameInput.name)
        .mockImplementation(() => {});
    ...
    test("#onButtonClick should show alert message if name is invalid", () => {
      ...
      expect(window.alert).toHaveBeenCalledWith("Campo obrigatório!");
    });
  });
  ...
  test("#initialize should call configureButton and updateTableWithCharactersFromAPI", async () => {
    const appInstance = new App({});
    jest
      .spyOn(appInstance, appInstance.updateTableWithCharactersFromAPI.name)
      .mockResolvedValue();
    jest
      .spyOn(appInstance, appInstance.configureButton.name)
      .mockResolvedValue();
